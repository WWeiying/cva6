...
module cva6_fifo_v4 #(
    ...
    input  dtype                  data_i2,      // data to push into the queue
    input  logic                  push_i2,      // data is valid and can be pushed to the queue
  ...
  if (DEPTH == 0) begin : gen_pass_through
    assign empty_o = ~(push_i || push_i2);
    assign full_o  = ~pop_i;
  end else begin : gen_fifo
    assign full_o  = (status_cnt_q >= FifoDepth[ADDR_DEPTH:0]-1);
    assign empty_o = (status_cnt_q == 0) & ~(FALL_THROUGH & (push_i || push_i2));
  end
  // status flags

  logic [1:0] push_count;
  logic [(ADDR_DEPTH-1):0] write_pointer_n_plus_1;
  assign write_pointer_n_plus_1 = write_pointer_q + 1;
  ...
  always_comb begin : read_write_comb
    ...
    push_count = 0;
    if (push_i && ~full_o) push_count = push_count + 1;
    if (push_i2 && ~full_o && status_cnt_q <= FifoDepth[ADDR_DEPTH:0]-2) push_count = push_count + 1;

    // push a new element to the queue
    if ((push_i || push_i2) && ~full_o) begin
      if (FPGA_EN) begin
        fifo_ram_we = 1'b1;
        ...
        if(push_count == 1) begin
          mem_n[write_pointer_q] = (data_i & {$bits(data_i){push_i}}) | (data_i2 & {$bits(data_i){push_i2}});
        end
        else begin
          mem_n[write_pointer_q] = data_i;
          mem_n[write_pointer_n_plus_1] = data_i2;
        end
      ...
      write_pointer_n = write_pointer_q + push_count;
      // increment the overall counter
      status_cnt_n = status_cnt_q + push_count;
    end
    ...
    if ((push_count > 0) && pop_i && ~full_o && ~empty_o) status_cnt_n = status_cnt_q + push_count - 1;

    // FIFO is in pass through mode -> do not change the pointers
    if ((FALL_THROUGH || (FPGA_EN && FPGA_ALTERA)) && (status_cnt_q == 0) && (push_i || push_i2)) begin
      if (FALL_THROUGH) begin
        if(push_count == 1) begin
          data_o = (data_i & {$bits(data_i){push_i}}) | (data_i2 & {$bits(data_i){push_i2}});
          if (pop_i) begin
            first_word_n = '0;
            status_cnt_n = status_cnt_q;
            read_pointer_n = read_pointer_q;
          ...
          if (pop_i) begin
            ...
            write_pointer_n = write_pointer_q + 2;
          end
        end
      end
...
//      end
  ...
  full_write :
  assert property (@(posedge clk_i) disable iff (~rst_ni) (full_o |-> ~(push_i || push_i2)))
  else begin 
    $display("Assertion failed at time %0t:", $time);
    $display("full_o = %b, push_i = %b, push_i2 = %b", full_o, push_i, push_i2);
